...
from pages.tracker import add_transaction, get_balance, get_expenses, plot_doughnut_chart,get_category_expenses
...
def tracker_page():
    ...
    if st.button("show plots"):
       if category_expenses is not None:
        ...
        st.pyplot(plot_doughnut_chart(category_expenses, "Overall Expense Distribution"))

        for cat in ["Food", "Clothes", "Entertainment", "Other"]:
                category_data = [t for t in get_expenses() if t["Category"] == cat]
                if category_data:
                    st.subheader(f"{cat} Expenses")
                    df = pd.DataFrame(category_data)
                    category_sums = df.groupby("Description")["Amount"].sum()
                    st.pyplot(plot_doughnut_chart(category_sums, f"{cat} Expense Breakdown"))
