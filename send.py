...
personal_advice_prompt = PromptTemplate(
    input_variables=["query", "chat_history"],
    template=""" 
    You are a personal advisor with expertise in career growth, relationships, mental well-being, and self-improvement.
    
    🔹 **User's Previous Conversations**: {chat_history}
    🔹 **User's Current Question**: {query}
    
    Based on this, provide **thoughtful, empathetic, and actionable advice**.
    Avoid generic responses and offer practical solutions.
    """
)
...
def give_personal_advice(query):
    ...
    prompt = personal_advice_prompt.format(
        query=query,
